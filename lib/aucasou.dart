...
class PuzzleNotifier extends StateNotifier<PuzzleState> {
  ...
  Future<Uint8List> generateCurrentImage() async {
    ...
    final width = state.imageSize.width.toInt();
    final height = state.imageSize.height.toInt();
    var currentImage = img.Image(width: width, height: height);
    int pieceWidth = (width / state.columns).ceil();
    int pieceHeight = (height / state.rows).ceil();

    for (int i = 0; i < state.currentArrangement.length; i++) {
      var pieceImage =
      img.decodeImage(state.pieces[state.currentArrangement[i]]);
      if (pieceImage != null) {
        int x = (i % state.columns) * pieceWidth;
        int y = (i ~/ state.columns) * pieceHeight;

        for (int py = 0; py < pieceHeight && y + py < height; py++) {
          for (int px = 0; px < pieceWidth && x + px < width; px++) {
            if (px < pieceImage.width && py < pieceImage.height) {
              currentImage.setPixel(
                  x + px, y + py, pieceImage.getPixel(px, py));
            }
          }
        }
      }
  ...
  Future<void> initializePuzzle(
    ...
    // Initialiser columns et rows avec des valeurs par défaut
    */
/*  int columns = 4; // Valeur par défaut
    int rows = (columns / aspectRatio).round();
    double bestError = double.infinity;

    for (int testColumns in [3, 4, 5]) {
...
*//*
    ...
    // Redimensionner l'image pour qu'elle soit divisible en pièces carrées
    const pieceSize = 100; // Taille fixe pour chaque pièce
    ...
    detailedTimes['resizing'] = resizeStopwatch.elapsed;

    // Créer les pièces du puzzle
    final piecesStopwatch = Stopwatch()..start();
    final pieces = _createPuzzlePieces({
      'image': resizedImage,
      'rows': rows,
      'columns': columns,
      'pieceSize': pieceSize,
  ...
  // Méthode pour mélanger toutes les pièces du puzzle
  ...
  // Méthode pour échanger deux pièces du puzzle
  ...
  // Extrayez la logique de détermination des colonnes/lignes dans une fonction séparée
...
/*  List<Uint8List> _createPuzzlePieces(Map<String, dynamic> params) {
    ...
    for (var j = 0; j < rows; j++) {
      for (var i = 0; i < columns; i++) {
        final x = (i * image.width / columns).round();
        final y = (j * image.height / rows).round();
        final width = ((i + 1) * image.width / columns).round() - x;
        final height = ((j + 1) * image.height / rows).round() - y;
        ...
        // Optionnel : redimensionner la pièce à une taille fixe si nécessaire
        final resizedPiece = img.copyResize(piece, width: pieceSize, height: pieceSize);

        pieces.add(Uint8List.fromList(img.encodePng(resizedPiece)));
      }
    }

    assert(pieces.length == rows * columns, "Le nombre de pièces ne correspond pas à la grille");
    return pieces;
  }*//*
